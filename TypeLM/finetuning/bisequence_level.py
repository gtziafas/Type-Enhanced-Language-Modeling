...
def tokenize_data(tokenizer: Tokenizer, data: Samples) -> List[Tuple[List[int], List[int], int]]:

    return [(tokenize_words(tokenizer.word_tokenizer, left),
             tokenize_words(tokenizer.word_tokenizer, right), tag) for left, right, tag in data]
...
class TypedLMForBiSequenceClassification(Module):
    def __init__(self, model_maker: Callable[[], TypedLM], num_classes: int, dropout_rate: float = 0.1):
        super(TypedLMForBiSequenceClassification, self).__init__()
        self.core = model_maker()
        self.classifier = Linear(in_features=2 * self.core.word_embedder.embedding_dim, out_features=num_classes)
        self.dropout = Dropout(dropout_rate)

    def forward(self, words: LongTensor, pad_mask: LongTensor) -> Tensor:
        deep = self.core.encode(words, pad_mask)[1]
        left, right = deep.chunk(2, dim=0)
        class_in = cat((left[:, 0], right[:, 0]), dim=-1)
        return self.classifier(self.dropout(class_in))
...
def sequence_collator(word_pad: int) -> Callable[[Samples], Tuple[LongTensor, LongTensor]]:
    def collate_fn(samples: Samples) -> Tuple[LongTensor, LongTensor]:
        ws1, ws2, ys = list(zip(*samples))
        return (_pad_sequence([tensor(w, dtype=long) for w in ws1 + ws2], batch_first=True, padding_value=word_pad),
                stack([tensor(y, dtype=long) for y in ys], dim=0))
    return collate_fn


def train_batch(model: TypedLMForBiSequenceClassification, loss_fn: Module, optim: Optimizer, words: LongTensor,
...
def train_epoch(model: TypedLMForBiSequenceClassification, loss_fn: Module, optim: Optimizer,
...
@no_grad()
def eval_batch(model: TypedLMForBiSequenceClassification, loss_fn: Module, words: LongTensor,
...
def eval_epoch(model: TypedLMForBiSequenceClassification, loss_fn: Module, dataloader: DataLoader,
